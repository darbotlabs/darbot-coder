	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"account:signIn": "Connect to darbot-coder Cloud",
	...
	it("should display benefits when user is not authenticated", () => {
		render(
			<AccountView
				...
				cloudApiUrl="https://github.com/DarbotLabs/darbot-coder"
				onDone={() => {}}
			/>,
		)

		// Check that the benefits section is displayed
		expect(screen.getByRole("heading", { name: "Connect to darbot-coder Cloud" })).toBeInTheDocument()
