	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 darbot-coder ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `darbot-coder ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 darbot-coder ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`darbot-coder ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
