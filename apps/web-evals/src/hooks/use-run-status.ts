...
import { type TokenUsage, DarbotCodeEventName, taskEventSchema } from "@darbot-code/types"
import type { Run, Task, TaskMetrics } from "@darbot-code/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case DarbotCodeEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case DarbotCodeEventName.TaskTokenUsageUpdated: {
			...
			case DarbotCodeEventName.EvalPass:
			case DarbotCodeEventName.EvalFail:
