...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@darbot-code/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"darbot-code-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"Darbot-Code-Nightly"',
			...
			setup(build) {
				build.onEnd(() => {
					...
					const generatedPackageJson = generatePackageJson({
						packageJson,
						overrideJson,
						substitution: ["darbot-coder", "darbot-code-nightly"],
					})
