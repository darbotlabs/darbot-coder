...
describe("Settings Migration", () => {
	...
	const legacyDarbotCustomModesPath = path.join(mockSettingsDir, "darbot_custom_modes.json")
	const legacyMcpSettingsPath = path.join(mockSettingsDir, "darbot_mcp_settings.json")
	...
	it("should migrate custom modes file if old file exists and new file doesn't", async () => {
		...
		vitest.mocked(fileExistsAtPath).mockImplementation(async (path: string) => {
			...
			if (path === legacyDarbotCustomModesPath) return true
		...
		// Verify expected rename call - darbot_custom_modes.json should be renamed to custom_modes.json
		expect(mockRename).toHaveBeenCalledWith(legacyDarbotCustomModesPath, legacyCustomModesJson)
	})
	...
	it("should migrate MCP settings file if old file exists and new file doesn't", async () => {
		...
		vitest.mocked(fileExistsAtPath).mockImplementation(async (path: string) => {
			...
			if (path === legacyDarbotCustomModesPath) return false // Ensure this file doesn't exist
	...
	it("should convert custom_modes.json to YAML format", async () => {
		...
		vitest.mocked(fileExistsAtPath).mockImplementation(async (path: string) => {
			...
			if (path === legacyDarbotCustomModesPath) return false
