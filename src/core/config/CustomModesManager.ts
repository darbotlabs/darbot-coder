...
import { type ModeConfig, type PromptComponent, customModesSettingsSchema, modeConfigSchema } from "@darbot-code/types"

import { fileExistsAtPath } from "../../utils/fs"
import { getWorkspacePath } from "../../utils/path"
import { getGlobalDarbotDirectory } from "../../services/darbot-config"
...
export class CustomModesManager {
	...
	private async getWorkspaceDarbotmodes(): Promise<string | undefined> {
	...
	private parseYamlSafely(content: string, filePath: string): any {
		...
		try {
			...
			// For .darbotmodes files, try JSON as fallback
			if (filePath.endsWith(DARBOTMODES_FILENAME)) {
				try {
			...
			// For non-.darbotmodes files, just log and return empty object
			const errorMsg = yamlError instanceof Error ? yamlError.message : String(yamlError)
			console.error(`[CustomModesManager] Failed to parse YAML from ${filePath}:`, errorMsg)
			return {}
		}
	...
	private async loadModesFromFile(filePath: string): Promise<ModeConfig[]> {
		try {
			...
			if (!result.success) {
				...
				// Show user-friendly error for .darbotmodes files
				if (filePath.endsWith(DARBOTMODES_FILENAME)) {
			...
			const isDarbotmodes = filePath.endsWith(DARBOTMODES_FILENAME)
			const source = isDarbotmodes ? ("project" as const) : ("global" as const)
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		const handleSettingsChange = async () => {
			try {
				...
				// Get modes from .darbotmodes if it exists (takes precedence)
				const darbotmodesPath = await this.getWorkspaceDarbotmodes()
				const darbotmodesModes = darbotmodesPath ? await this.loadModesFromFile(darbotmodesPath) : []

				// Merge modes from both sources (.darbotmodes takes precedence)
				const mergedModes = await this.mergeCustomModes(darbotmodesModes, result.data.customModes)
		...
		if (workspaceFolders && workspaceFolders.length > 0) {
			...
			const handleDarbotmodesChange = async () => {
				try {
					...
					const darbotmodesModes = await this.loadModesFromFile(darbotmodesPath)
					// .darbotmodes takes precedence
					const mergedModes = await this.mergeCustomModes(darbotmodesModes, settingsModes)
					await this.context.globalState.update("customModes", mergedModes)
					this.clearCache()
					await this.onUpdate()
				} catch (error) {
					console.error(`[CustomModesManager] Error handling .darbotmodes file change:`, error)
				}
			}

			this.disposables.push(darbotmodesWatcher.onDidChange(handleDarbotmodesChange))
			this.disposables.push(darbotmodesWatcher.onDidCreate(handleDarbotmodesChange))
			this.disposables.push(
				darbotmodesWatcher.onDidDelete(async () => {
					// When .darbotmodes is deleted, refresh with only settings modes
					try {
						const settingsModes = await this.loadModesFromFile(settingsPath)
						...
						console.error(`[CustomModesManager] Error handling .darbotmodes file deletion:`, error)
					}
				}),
			)
			this.disposables.push(darbotmodesWatcher)
		}
	}
	...
	public async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .darbotmodes if it exists.
		const darbotmodesPath = await this.getWorkspaceDarbotmodes()
		const darbotmodesModes = darbotmodesPath ? await this.loadModesFromFile(darbotmodesPath) : []
		...
		for (const mode of darbotmodesModes) {
		...
		const mergedModes = [
			...darbotmodesModes.map((mode) => ({ ...mode, source: "project" as const })),
			...settingsModes
	...
	public async deleteCustomMode(slug: string): Promise<void> {
		try {
			...
			const projectMode = darbotmodesModes.find((m) => m.slug === slug)
			...
			await this.queueWrite(async () => {
				...
				if (projectMode && darbotmodesPath) {
					await this.updateModesInFile(darbotmodesPath, (modes) => modes.filter((m) => m.slug !== slug))
				}
	...
	/**
	 * Checks if a mode has associated rules files in the .darbot/rules-{slug}/ directory
	...
	public async checkRulesDirectoryHasContent(slug: string): Promise<boolean> {
		try {
			...
			// Check if .darbotmodes file exists and contains this mode
			// This ensures we can only consolidate rules for modes that have been customized
			const darbotmodesPath = path.join(workspacePath, DARBOTMODES_FILENAME)
			try {
				const darbotmodesExists = await fileExistsAtPath(darbotmodesPath)
				if (darbotmodesExists) {
					const darbotmodesContent = await fs.readFile(darbotmodesPath, "utf-8")
					const darbotmodesData = yaml.parse(darbotmodesContent)
					const darbotmodesModes = darbotmodesData?.customModes || []

					// Check if this specific mode exists in .darbotmodes
					const modeInDarbotmodes = darbotmodesModes.find((m: any) => m.slug === slug)
					if (!modeInDarbotmodes) {
						return false // Mode not customized in .darbotmodes, cannot consolidate
					}
				} else {
					// If no .darbotmodes file exists, check if it's in global custom modes
				...
				// If we can't read .darbotmodes, fall back to checking custom modes
				const allModes = await this.getCustomModes()
				const mode = allModes.find((m) => m.slug === slug)
			...
			// Check for .darbot/rules-{slug}/ directory
			const modeRulesDir = path.join(workspacePath, ".darbot", `rules-${slug}`)
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
			...
			// Check if .darbotmodes file exists and contains this mode that has been customized
			if (!mode) {
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
				...
				try {
					...
					if (darbotmodesExists) {
						...
						// Find the mode in .darbotmodes
						mode = darbotmodesModes.find((m: any) => m.slug === slug)
					}
				} catch (error) {
				...
				if (stats.isDirectory()) {
					...
					for (const entry of entries) {
						if (entry.isFile()) {
							...
							const content = await fs.readFile(filePath, "utf-8")
							if (content.trim()) {
								// Calculate relative path from .darbot directory
								const relativePath = path.relative(path.join(workspacePath, ".darbot"), filePath)
		...
		try {
			...
			for (const importMode of importData.customModes) {
				...
				if (source === "project") {
					...
					const rulesFolderPath = path.join(workspacePath, ".darbot", `rules-${importMode.slug}`)
					try {
					...
					if (rulesFiles && Array.isArray(rulesFiles) && rulesFiles.length > 0) {
						...
						for (const ruleFile of rulesFiles) {
							if (ruleFile.relativePath && ruleFile.content) {
								...
								const targetPath = path.join(workspacePath, ".darbot", normalizedRelativePath)
								const normalizedTargetPath = path.normalize(targetPath)
								const expectedBasePath = path.normalize(path.join(workspacePath, ".darbot"))

								// Ensure the resolved path stays within the .darbot directory
					...
					// For global imports, preserve the rules files structure in the global .darbot directory
					const globalDarbotDir = getGlobalDarbotDirectory()

					// Always remove the existing rules folder for this mode if it exists
					// This ensures that if the imported mode has no rules, the folder is cleaned up
					const rulesFolderPath = path.join(globalDarbotDir, `rules-${importMode.slug}`)
					try {
						await fs.rm(rulesFolderPath, { recursive: true, force: true })
					...
					for (const ruleFile of rulesFiles) {
						if (ruleFile.relativePath && ruleFile.content) {
							...
							const targetPath = path.join(globalDarbotDir, normalizedRelativePath)
							const normalizedTargetPath = path.normalize(targetPath)
							const expectedBasePath = path.normalize(globalDarbotDir)

							// Ensure the resolved path stays within the global .darbot directory
							if (!normalizedTargetPath.startsWith(expectedBasePath)) {
								logger.error(`Path traversal attempt detected: ${ruleFile.relativePath}`)
								continue // Skip this file but continue with others
							}
