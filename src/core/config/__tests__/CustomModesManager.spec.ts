...
describe("CustomModesManager", () => {
	...
	const mockDarbotmodes = path.join(mockWorkspacePath, ".darbotmodes")
	...
	beforeEach(() => {
		...
		;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
			return path === mockSettingsPath || path === mockDarbotmodes
		})
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .darbotmodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const darbotmodesModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: settingsModes })
				}
				if (path === mockDarbotmodes) {
					return yaml.stringify({ customModes: darbotmodesModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .darbotmodes taking precedence", async () => {
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from darbotmodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .darbotmodes since it takes precedence
		...
		it("should handle missing .darbotmodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .darbotmodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .darbotmodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .darbotmodes precedence", async () => {
			...
			let darbotmodesContent = { customModes: darbotmodesModes }

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockDarbotmodes) {
					return yaml.stringify(darbotmodesContent)
				}
				if (path === mockSettingsPath) {
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockDarbotmodes) {
					darbotmodesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .darbotmodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Darbotmodes Mode 1", // .darbotmodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .darbotmodes file when adding project-specific mode", async () => {
			...
			// Mock .darbotmodes to not exist initially
			let darbotmodesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .darbotmodes was created with the project mode
			...
			const writeCall = (fs.writeFile as Mock).mock.calls[0]
			expect(path.normalize(writeCall[0])).toBe(path.normalize(mockDarbotmodes))

			// Verify the content written to .darbotmodes
			expect(darbotmodesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				;(fs.readFile as Mock).mockImplementation(async (path: string) => {
					...
					if (path === mockDarbotmodes && darbotmodesContent) {
						return yaml.stringify(darbotmodesContent)
					}
					throw new Error("File not found")
				})
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".darbotmodes"),
			...
			it("should import multiple modes at once", async () => {
				...
				expect(result.success).toBe(true)
				expect(darbotmodesContent.customModes).toHaveLength(2)
				expect(darbotmodesContent.customModes[0].slug).toBe("mode1")
				expect(darbotmodesContent.customModes[1].slug).toBe("mode2")
			})
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .darbotmodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .darbot directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".darbotmodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".darbot"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".darbot", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .darbotmodes file", async () => {
			const darbotmodesContent = { customModes: [{ slug: "other-mode", name: "Other Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockDarbotmodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockDarbotmodes) {
					return yaml.stringify(darbotmodesContent)
		...
		it("should return false when .darbotmodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when rules directory doesn't exist", async () => {
			const darbotmodesContent = { customModes: [{ slug: "test-mode", name: "Test Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockDarbotmodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.darbot/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .darbotmodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .darbotmodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .darbotmodes", async () => {
			const darbotmodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .darbotmodes", async () => {
			const darbotmodesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.darbot/rules-code" },
			])
