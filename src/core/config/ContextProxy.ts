...
type DarbotCodeSettingsKey = keyof DarbotCodeSettings
...
export class ContextProxy {
	...
	public setValue<K extends DarbotCodeSettingsKey>(key: K, value: DarbotCodeSettings[K]) {
		return isSecretStateKey(key) ? this.storeSecret(key, value as string) : this.updateGlobalState(key, value)
	}

	public getValue<K extends DarbotCodeSettingsKey>(key: K): DarbotCodeSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key) as DarbotCodeSettings[K])
			: (this.getGlobalState(key) as DarbotCodeSettings[K])
	}

	public getValues(): DarbotCodeSettings {
		return { ...this.getAllGlobalState(), ...this.getAllSecretState() }
	}

	public async setValues(values: DarbotCodeSettings) {
		const entries = Object.entries(values) as [DarbotCodeSettingsKey, unknown][]
	...
	public async export(): Promise<GlobalSettings | undefined> {
		try {
			...
			// Exports should only contain global settings, so this skips project custom modes (those exist in the .darbotmode folder)
