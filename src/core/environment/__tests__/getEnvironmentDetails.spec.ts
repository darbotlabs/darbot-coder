...
describe("getEnvironmentDetails", () => {
	...
	beforeEach(() => {
		...
		mockDarbot = {
			...
			darbotIgnoreController: {
				...
				dispose: vi.fn(),
			} as unknown as DarbotIgnoreController,
			darbotMessages: [],
			api: {
			...
			} as unknown as WeakRef<DarbotProvider>,
		}
	...
	it("should return basic environment details", async () => {
		const result = await getEnvironmentDetails(mockDarbot as Task)
		...
		expect(getApiMetrics).toHaveBeenCalledWith(mockDarbot.darbotMessages)
	})

	it("should include file details when includeFileDetails is true", async () => {
		const result = await getEnvironmentDetails(mockDarbot as Task, true)
	...
	it("should not include file details when includeFileDetails is false", async () => {
		await getEnvironmentDetails(mockDarbot as Task, false)
	...
	it("should include recently modified files if any", async () => {
		;(mockDarbot.fileContextTracker!.getAndClearRecentlyModifiedFiles as Mock).mockReturnValue([
			"modified1.ts",
			"modified2.ts",
		])
	...
	it("should include active terminal information", async () => {
		...
		mockDarbot.didEditFile = true
		await getEnvironmentDetails(mockDarbot as Task)
	...
	it("should handle missing provider or state", async () => {
		...
		mockDarbot.providerRef!.deref = vi.fn().mockReturnValue(null)
		...
		mockDarbot.providerRef!.deref = vi.fn().mockReturnValue({
			getState: vi.fn().mockResolvedValue(null),
		})

		const result2 = await getEnvironmentDetails(mockDarbot as Task)
	...
	it("should handle errors gracefully", async () => {
		...
		await expect(getEnvironmentDetails(mockDarbot as Task)).resolves.not.toThrow()
	})
})
