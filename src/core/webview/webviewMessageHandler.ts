		...
		case "deleteCustomMode":
			if (message.slug) {
				...
				if (scope === "project") {
					const workspacePath = getWorkspacePath()
					if (workspacePath) {
						rulesFolderPath = path.join(workspacePath, ".darbot", `rules-${message.slug}`)
					} else {
						rulesFolderPath = path.join(".darbot", `rules-${message.slug}`)
					}
				} else {
					// Global scope - use OS home directory
					const homeDir = os.homedir()
					rulesFolderPath = path.join(homeDir, ".darbot", `rules-${message.slug}`)
				}
