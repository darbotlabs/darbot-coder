// npx vitest core/webview/__tests__/DarbotProvider.spec.ts

import Anthropic from "@anthropic-ai/sdk"
import * as vscode from "vscode"
import axios from "axios"

import { type ProviderSettingsEntry, type DarbotMessage, ORGANIZATION_ALLOW_ALL } from "@darbot-code/types"
...
vi.mock("@darbot-code/cloud", () => ({
	...
	getDarbotCodeApiUrl: vi.fn().mockReturnValue("https://app.darbotcode.com"),
}))
...
describe("DarbotProvider", () => {
	...
	beforeEach(() => {
		...
		provider = new DarbotProvider(mockContext, mockOutputChannel, "sidebar", new ContextProxy(mockContext))
	...
	test("constructor initializes correctly", () => {
		expect(provider).toBeInstanceOf(DarbotProvider)
		// Since getVisibleInstance returns the last instance where view.visible is true
		// @ts-ignore - accessing private property for testing
		provider.view = mockWebviewView
		expect(DarbotProvider.getVisibleInstance()).toBe(provider)
	})
	...
	test("clearTask aborts current task", async () => {
		// Setup Darbot instance with auto-mock from the top of the file
		const mockDarbot = new Task(defaultTaskOptions) // Create a new mocked instance

		// add the mock object to the stack
		await provider.addDarbotToStack(mockDarbot)

		// get the stack size before the abort call
		const stackSizeBeforeAbort = provider.getDarbotStackSize()

		// call the removeDarbotFromStack method so it will call the current darbot abort and remove it from the stack
		await provider.removeDarbotFromStack()

		// get the stack size after the abort call
		const stackSizeAfterAbort = provider.getDarbotStackSize()

		// check if the abort method was called
		expect(mockDarbot.abortTask).toHaveBeenCalled()
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask when there is no parent task", async () => {
			...
			const mockDarbot = new Task(defaultTaskOptions)
		...
		test("calls finishSubTask when there is a parent task", async () => {
			...
			;(childTask as any).darbottTask = parentTask
			...
			await provider.addDarbotToStack(parentTask)
			await provider.addDarbotToStack(childTask)
		...
		test("correctly identifies subtask scenario for issue #4602", async () => {
			...
			expect(provider.getDarbotStackSize()).toBe(1)
	...
	test("addDarbotToStack adds multiple Darbot instances to the stack", async () => {
		// Setup Darbot instance with auto-mock from the top of the file
		const mockDarbot1 = new Task(defaultTaskOptions) // Create a new mocked instance
		const mockDarbot2 = new Task(defaultTaskOptions) // Create a new mocked instance
		Object.defineProperty(mockDarbot1, "taskId", { value: "test-task-id-1", writable: true })
		Object.defineProperty(mockDarbot2, "taskId", { value: "test-task-id-2", writable: true })

		// add Darbot instances to the stack
		await provider.addDarbotToStack(mockDarbot1)
		await provider.addDarbotToStack(mockDarbot2)

		// verify darbot instances were added to the stack
		expect(provider.getDarbotStackSize()).toBe(2)

		// verify current darbot instance is the last one added
		expect(provider.getCurrentDarbot()).toBe(mockDarbot2)
	})
	...
	test("handles showDarbotIgnoredFiles setting", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as any).mock.calls[0][0]

		// Default value should be true
		expect((await provider.getState()).showDarbotIgnoredFiles).toBe(true)

		// Test showDarbotIgnoredFiles with true
		await messageHandler({ type: "showDarbotIgnoredFiles", bool: true })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showDarbotIgnoredFiles", true)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showDarbotIgnoredFiles).toBe(true)

		// Test showDarbotIgnoredFiles with false
		await messageHandler({ type: "showDarbotIgnoredFiles", bool: false })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showDarbotIgnoredFiles", false)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showDarbotIgnoredFiles).toBe(false)
	})
	...
	describe("deleteMessage", () => {
		...
		test("handles deletion with confirmation dialog", async () => {
			...
			const mockDarbot = new Task(defaultTaskOptions) // Create a new mocked instance
			mockDarbot.darbotMessages = mockMessages // Set test-specific messages
			mockDarbot.apiConversationHistory = mockApiHistory // Set API history
			await provider.addDarbotToStack(mockDarbot) // Add the mocked instance to the stack
			...
			// Mock initDarbotWithHistoryItem
			;(provider as any).initDarbotWithHistoryItem = vi.fn()
			...
			expect(mockDarbot.overwriteDarbotMessages).toHaveBeenCalledWith([mockMessages[0], mockMessages[1]])

			// Verify only API messages before the deleted message were kept
			expect(mockDarbot.overwriteApiConversationHistory).toHaveBeenCalledWith([
				mockApiHistory[0],
				mockApiHistory[1],
			])

			// Verify initDarbotWithHistoryItem was called
			expect((provider as any).initDarbotWithHistoryItem).toHaveBeenCalledWith({ id: "test-task-id" })
		})

		test("handles case when no current task exists", async () => {
			// Clear the darbot stack
			;(provider as any).darbotStack = []

			// Trigger message deletion
			const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as any).mock.calls[0][0]
			await messageHandler({ type: "deleteMessage", value: 2000 })

			// Verify no dialog was shown since there's no current darbot
	...
	describe("editMessage", () => {
		...
		test("handles edit with confirmation dialog", async () => {
			...
			mockDarbot.overwriteDarbotMessages = vi.fn()
			mockDarbot.overwriteApiConversationHistory = vi.fn()
			mockDarbot.handleWebviewAskResponse = vi.fn()
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.darbot", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.darbot/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.darbot/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			expect.stringContaining("Failed to create or open .darbot/mcp.json"),
		)
	})
})
...
describe.skip("ContextProxy integration", () => {
	...
	beforeEach(() => {
		...
		provider = new DarbotProvider(mockContext, mockOutputChannel, "sidebar", mockContextProxy)
	})
...
describe("getTelemetryProperties", () => {
	...
	beforeEach(() => {
		...
		mockDarbot = new Task(defaultTaskOptions)
		mockDarbot.api = {
			getModel: vi.fn().mockReturnValue({
	...
	test("includes model ID from current Darbot instance if available", async () => {
		// Add mock Darbot to stack
		await provider.addDarbotToStack(mockDarbot)
	...
	describe("cloud authentication telemetry", () => {
		...
		test("includes cloud authentication property when user is authenticated", async () => {
			...
			const { CloudService } = await import("@darbot-code/cloud")
...
describe("DarbotProvider - Router Models", () => {
	let provider: DarbotProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
...
describe("DarbotProvider - Comprehensive Edit/Delete Edge Cases", () => {
	let provider: DarbotProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	...
	describe("Edit Messages with Images and Attachments", () => {
		...
		test("handles editing messages containing images", async () => {
			...
			const mockDarbot = new Task(defaultTaskOptions)
			mockDarbot.darbotMessages = mockMessages
			mockDarbot.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }] as any[]
			mockDarbot.overwriteDarbotMessages = vi.fn()
			mockDarbot.overwriteApiConversationHistory = vi.fn()
			mockDarbot.handleWebviewAskResponse = vi.fn()
			...
			expect(mockDarbot.overwriteDarbotMessages).toHaveBeenCalledWith([mockMessages[0]])
			expect(mockDarbot.overwriteApiConversationHistory).toHaveBeenCalledWith([{ ts: 1000 }])
		})
		...
		test("handles editing messages with file attachments", async () => {
			...
			expect(mockDarbot.overwriteDarbotMessages).toHaveBeenCalled()
			expect(mockDarbot.handleWebviewAskResponse).toHaveBeenCalledWith(
				"messageResponse",
	...
	describe("Network Failure Scenarios", () => {
		...
		test("handles network timeout during edit submission", async () => {
			const mockDarbot = new Task(defaultTaskOptions)
			mockDarbot.darbotMessages = [
				{ ts: 1000, type: "say", say: "user_feedback", text: "Original message", value: 2000 },
				{ ts: 2000, type: "say", say: "text", text: "AI response" },
			] as DarbotMessage[]
			mockDarbot.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockDarbot.overwriteDarbotMessages = vi.fn()
			mockDarbot.overwriteApiConversationHistory = vi.fn()
			mockDarbot.handleWebviewAskResponse = vi.fn().mockRejectedValue(new Error("Network timeout"))
		...
		test("handles connection drops during edit operation", async () => {
			...
			mockDarbot.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockDarbot.overwriteDarbotMessages = vi.fn().mockRejectedValue(new Error("Connection lost"))
			mockDarbot.overwriteApiConversationHistory = vi.fn()
			mockDarbot.handleWebviewAskResponse = vi.fn()
	...
	describe("Concurrent Edit Operations", () => {
		...
		test("handles race conditions with simultaneous edits", async () => {
			...
			mockDarbot.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }, { ts: 4000 }] as any[]
			mockDarbot.overwriteDarbotMessages = vi.fn()
			mockDarbot.overwriteApiConversationHistory = vi.fn()
			mockDarbot.handleWebviewAskResponse = vi.fn()
	...
	describe("Edit Permissions and Authorization", () => {
		...
		test("handles edit permission failures", async () => {
			// Mock no current darbot (simulating permission failure)
			vi.spyOn(provider, "getCurrentDarbot").mockReturnValue(undefined)
			...
			// Should not show confirmation dialog when no current darbot
		...
		test("handles authorization failures during edit", async () => {
			...
			mockDarbot.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockDarbot.overwriteDarbotMessages = vi.fn().mockRejectedValue(new Error("Unauthorized"))
			mockDarbot.overwriteApiConversationHistory = vi.fn()
			mockDarbot.handleWebviewAskResponse = vi.fn()
		...
		describe("Operations on Deleted or Non-existent Messages", () => {
			...
			test("handles edit operations on deleted messages", async () => {
				...
				mockDarbot.apiConversationHistory = [{ ts: 1000 }] as any[]
				mockDarbot.overwriteDarbotMessages = vi.fn()
				mockDarbot.overwriteApiConversationHistory = vi.fn()
				mockDarbot.handleWebviewAskResponse = vi.fn()
				...
				expect(mockDarbot.overwriteDarbotMessages).not.toHaveBeenCalled()
				expect(mockDarbot.handleWebviewAskResponse).not.toHaveBeenCalled()
			})
		...
		describe("Resource Cleanup During Failed Operations", () => {
			...
			test("validates proper cleanup during failed edit operations", async () => {
				...
				mockDarbot.overwriteDarbotMessages = vi.fn().mockImplementation(() => {
					cleanupSpy()
		...
		describe("Error Messaging and User Feedback", () => {
			...
			test("provides user feedback for successful operations", async () => {
				...
				expect(mockDarbot.overwriteDarbotMessages).toHaveBeenCalled()
				expect(provider.initDarbotWithHistoryItem).toHaveBeenCalled()
			...
			test("handles user cancellation gracefully", async () => {
				...
				expect(mockDarbot.overwriteDarbotMessages).not.toHaveBeenCalled()
				expect(mockDarbot.overwriteApiConversationHistory).not.toHaveBeenCalled()
				expect(mockDarbot.handleWebviewAskResponse).not.toHaveBeenCalled()
				expect(vscode.window.showErrorMessage).not.toHaveBeenCalled()
			})
		})
		...
		describe("Edge Cases with Message Timestamps", () => {
			...
			test("handles messages with identical timestamps", async () => {
				...
				mockDarbot.apiConversationHistory = [{ ts: 1000 }, { ts: 1000 }, { ts: 1000 }, { ts: 2000 }] as any[]
				mockDarbot.overwriteDarbotMessages = vi.fn()
				mockDarbot.overwriteApiConversationHistory = vi.fn()
			...
			test("handles messages with future timestamps", async () => {
				...
				mockDarbot.apiConversationHistory = [
					{ ts: 1000 },
					{ ts: futureTimestamp },
				...
				expect(mockDarbot.overwriteDarbotMessages).toHaveBeenCalled()
				expect(mockDarbot.handleWebviewAskResponse).toHaveBeenCalled()
			})
		})
	})
})
