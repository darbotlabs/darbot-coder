...
import { DarbotProtectedController } from "../DarbotProtectedController"

describe("DarbotProtectedController", () => {
	const TEST_CWD = "/test/workspace"
	let controller: DarbotProtectedController

	beforeEach(() => {
		controller = new DarbotProtectedController(TEST_CWD)
	})
	...
	describe("isWriteProtected", () => {
		...
		it("should not protect files that contain 'darbot' but don't start with .darbot", () => {
			expect(controller.isWriteProtected("src/darbot-utils.ts")).toBe(false)
			expect(controller.isWriteProtected("config/darbot.config.js")).toBe(false)
		})
	...
	describe("getProtectionMessage", () => {
		it("should return appropriate protection message", () => {
			...
			expect(message).toBe("This is a Darbot configuration file and requires approval for modifications")
		})
	})
	...
	describe("getProtectedPatterns", () => {
		it("should return the list of protected patterns", () => {
			const patterns = DarbotProtectedController.getProtectedPatterns()
