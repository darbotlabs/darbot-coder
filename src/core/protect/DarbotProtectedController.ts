...
/**
 * Controls write access to Darbot configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive Darbot configuration files.
 */
export class DarbotProtectedController {
	private cwd: string
	private ignoreInstance: Ignore

	// Predefined list of protected Darbot configuration patterns
	private static readonly PROTECTED_PATTERNS = [
		...
		".darbot/**",
		".vscode/**",
		".darbotprotected", // For future use
	]
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(DarbotProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a Darbot configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = DarbotProtectedController.PROTECTED_PATTERNS.join(", ")
		return `# Protected Files\n\n(The following Darbot configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return DarbotProtectedController.PROTECTED_PATTERNS
	}
}
