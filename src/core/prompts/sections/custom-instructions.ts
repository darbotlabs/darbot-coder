...
export async function loadRuleFiles(cwd: string): Promise<string> {
	...
	// If we found rules in .darbot/rules/ directories, return them
	if (rules.length > 0) {
		return "\n" + rules.join("\n\n")
	}
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		// If we found mode-specific rules in .darbot/rules-${mode}/ directories, use them
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".darbot", `rules-${mode}`))) {
	...
	if (options.darbotIgnoreInstructions) {
		rules.push(options.darbotIgnoreInstructions)
	}
