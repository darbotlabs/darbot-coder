// npx vitest core/prompts/__tests__/responses-darbotignore.spec.ts
...
describe("DarbotIgnore Response Formatting", () => {
	...
	describe("formatResponse.darbotIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.darbotIgnoreError("secrets/api-keys.json")

			// Verify error message format
			expect(errorMessage).toContain("Access to secrets/api-keys.json is blocked by the .darbotignore file settings")
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .darbotignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.darbotIgnoreError(testPath)
				expect(errorMessage).toContain(`Access to ${testPath} is blocked`)
			}
		})
	})

	describe("formatResponse.formatFilesList with DarbotIgnoreController", () => {
		/**
		 * Tests file listing with darbotignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new DarbotIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showDarbotIgnoredFiles is set to false
		 */
		it("should hide ignored files when showDarbotIgnoredFiles is false", async () => {
			// Create controller
			const controller = new DarbotIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showDarbotIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showDarbotIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with DarbotIgnoreController
		 */
		it("should handle truncation with DarbotIgnoreController", async () => {
			// Create controller
			const controller = new DarbotIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with DarbotIgnoreController", async () => {
			// Create controller
			const controller = new DarbotIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .darbotignore instructions for the LLM", async () => {
			// Create controller
			const controller = new DarbotIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .darbotignore exists", async () => {
			// Set up no .darbotignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .darbotignore
			const controller = new DarbotIgnoreController(TEST_CWD)
			await controller.initialize()
