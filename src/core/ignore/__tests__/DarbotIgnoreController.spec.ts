// npx vitest core/ignore/__tests__/DarbotIgnoreController.spec.ts

import type { Mock } from "vitest"

import { DarbotIgnoreController, LOCK_TEXT_SYMBOL } from "../DarbotIgnoreController"
...
describe("DarbotIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: DarbotIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new DarbotIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .darbotignore exists
		 */
		it("should load .darbotignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .darbotignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".darbotignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".darbotignore"), "utf8")

			// Verify content was stored
			expect(controller.darbotIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .darbotignore doesn't exist
		 */
		it("should allow all access when .darbotignore doesn't exist", async () => {
			// Setup mocks to simulate missing .darbotignore file
			...
			expect(controller.darbotIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .darbotignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".darbotignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .darbotignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .darbotignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .darbotignore exists
		 */
		it("should allow all access when no .darbotignore content", async () => {
			// Create a new controller with no .darbotignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new DarbotIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .darbotignore exists
		 */
		it("should allow all commands when no .darbotignore exists", async () => {
			// Create a new controller with no .darbotignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new DarbotIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .darbotignore
		 */
		it("should generate formatted instructions when .darbotignore exists", async () => {
			// Setup .darbotignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .darbotignore exists
		 */
		it("should return undefined when no .darbotignore exists", async () => {
			// Setup no .darbotignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .darbotignore is created
		 */
		it("should reload .darbotignore when file is created", async () => {
			// Setup initial state without .darbotignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .darbotignore
			controller = new DarbotIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .darbotignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .darbotignore is changed
		 */
		it("should reload .darbotignore when file is changed", async () => {
			// Setup initial state with .darbotignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .darbotignore is deleted
		 */
		it("should reset when .darbotignore is deleted", async () => {
			// Setup initial state with .darbotignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
