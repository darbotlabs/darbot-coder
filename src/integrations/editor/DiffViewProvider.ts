...
export const DIFF_VIEW_URI_SCHEME = "darbot-diff"
export const DIFF_VIEW_LABEL_CHANGES = "Original ↔ darbot's Changes"

// TODO: https://github.com/DarbotLabs/darbot-coder/pull/3354
export class DiffViewProvider {
	...
	async open(relPath: string): Promise<void> {
		...
		// after editing to see if darbot needs to fix anything.
	...
	async saveChanges(diagnosticsEnabled: boolean = true, writeDelayMs: number = DEFAULT_WRITE_DELAY_MS): Promise<{
		...
		// Since these are new problems resulting from darbot's edit, we know they're
		// directly related to the work it's doing. This eliminates the risk of darbot
		// going off-task or getting distracted by unrelated issues, which was a problem
		// with the previous auto-debug approach. Some users' machines may be slow to
		// update diagnostics, so this approach provides a good balance between automation
		// and avoiding potential issues where darbot might get stuck in loops due to
		// outdated problem information. If no new problems show up by the time the user
		// accepts the changes, they can always debug later using the '@problems' mention.
		// This way, darbot only becomes aware of new problems resulting from its edits
