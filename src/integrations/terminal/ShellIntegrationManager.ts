...
export class ShellIntegrationManager {
	...
	public static zshInitTmpDir(env: Record<string, string>): string {
		...
		const tmpDir = path.join(os.tmpdir(), `darbot-zdotdir-${Math.random().toString(36).substring(2, 15)}`)
		console.info(`[TerminalRegistry] Creating temporary directory for ZDOTDIR: ${tmpDir}`)

		// Save original ZDOTDIR as DARBOT_ZDOTDIR
		if (process.env.ZDOTDIR) {
			env.DARBOT_ZDOTDIR = process.env.ZDOTDIR
		}
	...
	ZDOTDIR=\${DARBOT_ZDOTDIR:-$HOME}
	unset DARBOT_ZDOTDIR
