...
import { DarbotTerminal, DarbotTerminalProvider } from "./types"
...
export class TerminalRegistry {
	private static terminals: DarbotTerminal[] = []
		...
		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					...
					if (terminal) {
						...
						console.error(
							"[onDidStartTerminalShellExecution] Shell execution started, but not from a darbot-registered terminal:",
							e,
						)
					}
				},
			...
			const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionEndEvent) => {
					...
					if (!terminal) {
						console.error(
							"[onDidEndTerminalShellExecution] Shell execution ended, but not from a darbot-registered terminal:",
							e,
						)
	...
	public static createTerminal(cwd: string, provider: DarbotTerminalProvider): DarbotTerminal {
		let newTerminal
	...
	public static async getOrCreateTerminal(
		...
		taskId?: string,
		provider: DarbotTerminalProvider = "vscode",
	): Promise<DarbotTerminal> {
		const terminals = this.getAllTerminals()
		let terminal: DarbotTerminal | undefined
	 ...
	 */
	public static getTerminals(busy: boolean, taskId?: string): DarbotTerminal[] {
	 ...
	 * @returns Array of Terminal objects
	 */
	public static getBackgroundTerminals(busy?: boolean): DarbotTerminal[] {
		return this.getAllTerminals().filter((t) => {
	...
	private static getAllTerminals(): DarbotTerminal[] {
		this.terminals = this.terminals.filter((t) => !t.isClosed())
		return this.terminals
	}

	private static getTerminalById(id: number): DarbotTerminal | undefined {
	 ...
	 */
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): DarbotTerminal | undefined {
