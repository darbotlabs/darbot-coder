...
vi.mock("@darbot-code/cloud", () => ({
	getDarbotCodeApiUrl: () => "https://test.api.com",
}))
	...
	describe("installMarketplaceItem", () => {
		it("should install a mode item", async () => {
			...
			vi.spyOn(manager["installer"], "installItem").mockResolvedValue({
				filePath: "/test/path/.darbotmodes",
				line: 5,
			})
			...
			expect(result).toBe("/test/path/.darbotmodes")
		})
		...
		it("should install an MCP item", async () => {
			...
			// Mock the installer
			vi.spyOn(manager["installer"], "installItem").mockResolvedValue({
				filePath: "/test/path/.darbot/mcp.json",
				line: 3,
			})
			...
			expect(manager["installer"].installItem).toHaveBeenCalledWith(item, { target: "project" })
			expect(result).toBe("/test/path/.darbot/mcp.json")
		})
	})
