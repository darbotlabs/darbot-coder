...
import type { MarketplaceItem, MarketplaceItemType } from "@darbot-code/types"
...
export class MarketplaceManager {
	...
	private async checkProjectInstallations(metadata: Record<string, { type: string }>): Promise<void> {
		try {
			...
			// Check modes in .darbotmodes
			const projectModesPath = path.join(workspaceFolder.uri.fsPath, ".darbotmodes")
			try {
			...
			// Check MCPs in .darbot/mcp.json
			const projectMcpPath = path.join(workspaceFolder.uri.fsPath, ".darbot", "mcp.json")
			try {
