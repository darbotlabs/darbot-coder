...
vi.mock("@darbot-code/cloud", () => ({
	...
	PRODUCTION_CLERK_BASE_URL: "https://clerk.darbotcode.com",
}))
...
vi.mock("../../../shared/package", () => ({
	Package: {
		publisher: "darbot-coder",
		name: "darbot-coder",
		version: "1.0.0",
		outputChannel: "Darbot-Coder",
		sha: undefined,
	},
}))
	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires Darbot Code Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's Darbot Code Cloud account.",
...
import { CloudService, getClerkBaseUrl, PRODUCTION_CLERK_BASE_URL } from "@darbot-code/cloud"
...
describe("MdmService", () => {
	...
	beforeEach(() => {
		...
		mockGetClerkBaseUrl.mockReturnValue("https://dev.clerk.darbotcode.com")
	...
	describe("platform-specific config paths", () => {
		...
		it("should use correct path for Windows in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "DarbotCode", "mdm.json"))
		})
		...
		it("should use correct path for Windows in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "DarbotCode", "mdm.dev.json"))
		})
		...
		it("should use correct path for macOS in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/DarbotCode/mdm.json")
		})
		...
		it("should use correct path for macOS in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/DarbotCode/mdm.dev.json")
		})
		...
		it("should use correct path for Linux in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/darbot-coder/mdm.json")
		})
		...
		it("should use correct path for Linux in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/darbot-coder/mdm.dev.json")
		})
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires Darbot Code Cloud authentication")
			}
		})
