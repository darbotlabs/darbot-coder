...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\DarbotCode\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "DarbotCode", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/DarbotCode/mdm.json or mdm.dev.json
				return `/Library/Application Support/DarbotCode/${configFileName}`
