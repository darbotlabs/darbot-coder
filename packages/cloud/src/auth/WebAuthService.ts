...
import type { CloudUserInfo, CloudOrganizationMembership } from "@darbot-code/types"

import { getClerkBaseUrl, getDarbotCodeApiUrl, PRODUCTION_CLERK_BASE_URL } from "../Config"
...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(): Promise<void> {
		try {
			...
			const publisher = packageJSON?.publisher ?? "DarbotLabs"
			const name = packageJSON?.name ?? "darbot-coder"
			const params = new URLSearchParams({
				state,
				auth_redirect: `${vscode.env.uriScheme}://${publisher}.${name}`,
			})
			const url = `${getDarbotCodeApiUrl()}/extension/sign-in?${params.toString()}`
			await vscode.env.openExternal(vscode.Uri.parse(url))
		} catch (error) {
			this.log(`[auth] Error initiating darbot-coder Cloud auth: ${error}`)
			throw new Error(`Failed to initiate darbot-coder Cloud authentication: ${error}`)
		}
	}

	/**
	 * Handle the callback from darbot-coder Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with darbot-coder Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			vscode.window.showInformationMessage("Invalid darbot-coder Cloud sign in url")
			return
		}
		...
		try {
			...
			vscode.window.showInformationMessage("Successfully authenticated with darbot-coder Cloud")
			this.log("[auth] Successfully authenticated with darbot-coder Cloud")
		} catch (error) {
			this.log(`[auth] Error handling darbot-coder Cloud callback: ${error}`)
			const previousState = this.state
			this.state = "logged-out"
			this.emit("logged-out", { previousState })
			throw new Error(`Failed to handle darbot-coder Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			vscode.window.showInformationMessage("Logged out from darbot-coder Cloud")
			this.log("[auth] Logged out from darbot-coder Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from darbot-coder Cloud: ${error}`)
			throw new Error(`Failed to log out from darbot-coder Cloud: ${error}`)
		}
	}
