...
import { getDarbotCodeApiUrl } from "./Config"
...
export class TelemetryClient extends BaseTelemetryClient {
	...
	private async fetch(path: string, options: RequestInit) {
		...
		const response = await fetch(`${getDarbotCodeApiUrl()}/api/${path}`, {
			...options,
	...
	public async backfillMessages(messages: ClineMessage[], taskId: string): Promise<void> {
		...
		try {
			...
			const response = await fetch(`${getDarbotCodeApiUrl()}/api/events/backfill`, {
				method: "POST",
				headers: {
