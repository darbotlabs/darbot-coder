...
import { type TaskEvent, TaskCommandName, DarbotCodeEventName, IpcMessageType, EVALS_SETTINGS } from "@darbot-code/types"
import { IpcClient } from "@darbot-code/ipc"
...
export const processTask = async ({ taskId, logger }: { taskId: number; logger?: Logger }) => {
	...
	try {
		...
		await publish({
			eventName: passed ? DarbotCodeEventName.EvalPass : DarbotCodeEventName.EvalFail,
			taskId: task.id,
		})
	} finally {
...
export const processTaskInContainer = async ({
	...
	const command = `pnpm --filter @darbot-code/evals cli --taskId ${taskId}`
	logger.info(command)
...
export const runTask = async ({ run, task, publish, logger }: RunTaskOptions) => {
	...
	const ignoreEvents: Record<"broadcast" | "log", DarbotCodeEventName[]> = {
		broadcast: [DarbotCodeEventName.Message],
		log: [DarbotCodeEventName.TaskTokenUsageUpdated, DarbotCodeEventName.TaskAskResponded],
	}
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		if (
			...
			(eventName !== DarbotCodeEventName.Message || payload[0].message.partial !== true)
		) {
			logger.info(`${eventName} ->`, payload)
		}

		if (eventName === DarbotCodeEventName.TaskStarted) {
		...
		if (
			(eventName === DarbotCodeEventName.TaskTokenUsageUpdated || eventName === DarbotCodeEventName.TaskCompleted) &&
			taskMetricsId
		) {
		...
		if (eventName === DarbotCodeEventName.TaskCompleted && taskMetricsId) {
			const toolUsage = payload[2]
			await updateTaskMetrics(taskMetricsId, { toolUsage })
		}

		if (eventName === DarbotCodeEventName.TaskAborted) {
			taskAbortedAt = Date.now()
		}

		if (eventName === DarbotCodeEventName.TaskCompleted) {
