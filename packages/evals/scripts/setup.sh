...
build_extension() {
  echo "🔨 Building the Darbot Code extension..."
  pnpm -w vsix -- --out ../bin/darbot-coder-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/darbot-coder-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "DarbotFramework.darbot-coder"; then
  code --install-extension DarbotFramework.darbot-coder &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing Darbot Code evals database... "
pnpm --filter @darbot-code/evals db:push --force &>/dev/null || exit 1
echo "✅ Done"
...
current_version=$(code --list-extensions --show-versions 2>/dev/null | grep darbot)
read -p "💻 Do you want to build a new version of the Darbot Code extension? [currently $current_version] (y/N): " build_extension
...
if ! nc -z localhost 3000; then
  ...
  if [[ "$start_evals" =~ ^[Yy]|^$ ]]; then
    pnpm --filter @darbot-code/web-evals dev
  else
    echo "💡 You can start it anytime with 'pnpm --filter @darbot-code/web-evals dev'."
  fi
else
